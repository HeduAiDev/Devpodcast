...
const tts = await agent(
  head(null, [
    ...
    '方式：用 scripts/tts.py 的 firered-tts2 provider（FireRedTTSProvider，temperature=0.8/topk=15，2026-08-07 人工试听 20 组选定）：python scripts/tts.py synthesize <script.md> --voice-map S1=<wav1>,S2=<wav2> --provider firered-tts2 --output <audio_dir> --target-minutes <N>。prompt_text 从 wav 同目录的 <name>.txt 读（需存在）。FireRed 自动动态分段（≤450 字/段，防超 max_seq_len），段间 350ms 拼接。注音替换自动生效（season/pronunciation.json，SGLang→SG浪 等）。',
