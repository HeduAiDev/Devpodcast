...
// 便于 args.models 覆盖，两处应保持一致。
const MODELS = Object.assign(
  { planner: 'sonnet', researcher: 'sonnet', hookEngineer: 'sonnet', bookAnalyst: 'sonnet', archivist: 'sonnet', runner: 'sonnet' },
  A.models || {},
)
...
const plan = await agent(
  ...
  { schema: PLAN_SCHEMA, label: 'plan', phase: 'Plan', agentType: 'planner', model: MODELS.planner },
)
...
const research = await agent(
  ...
  { schema: STATUS_SCHEMA, label: 'research', phase: 'Research', agentType: 'researcher', model: MODELS.researcher },
)
...
for (;;) {
  ...
  const fix = await agent(
    ...
    { schema: STATUS_SCHEMA, label: 'voices-fix r' + voicesFixes, phase: 'Research', agentType: 'researcher', model: MODELS.researcher },
  )
...
const hooks = await agent(
  ...
  { schema: STATUS_SCHEMA, label: 'hooks', phase: 'Hooks', agentType: 'hook-engineer', model: MODELS.hookEngineer },
)
...
const sliceThunks = EPS.map(function (ep) {
  return function () {
    ...
    return agent(
      ...
      { schema: STATUS_SCHEMA, label: 'slice ' + ep.slug, phase: 'Slice', agentType: 'book-analyst', model: MODELS.bookAnalyst },
    )
  }
})
