...
## 重读标记 {{em}}（2026-08-13 起可用，IndexTTS-2.5 合成）

关键术语需要**重读**（音高/时长强调，区别于 `<break>` 的停顿）时，用 `{{em}}词{{/em}}` 包住**词本身**（不带「」引号；标签会剥离，词留在正文）：

- **什么时候用**：术语首次定义处；对比结构（不是X，是Y）里的重点词；金句/格言。跨期核心词（大他者 / 欲望 / 划杠主体 / 缝合点 / 能指 / 无意识像语言 / 人的欲望是他者的欲望）每期首现必用。
- **纪律**：每 turn 至多 1 处；包核心术语不包整句（≤15 字，lint WARN）；不与 `<break>` 交叉（lint BLOCKING）。
- 合成器处理：发音表 g2p 注音（读准）+ 放慢 3% + 响度对齐前段——重读感来自拉长和加响，**不改变发音**（情绪向量会读歪词，禁用）。
- 例：`从{{em}}它思{{/em}}出发。`
- **辅助判据**：脚本里「你这话我得记下来」「这句先记下」「钉死它」这类元标记句子**之前/所在的句子** = 天然的 {{em}} 候选。
