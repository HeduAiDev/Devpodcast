...
- `<break Nms>` 显式停顿标记（N 为毫秒整数）——**句间转折/口头禅引导处必须用**（见下方节奏纪律）
- 首行 `# 标题`
...
1. **难点要慢**：核心机制/易混概念主动降速，给思考时间（`<break 500ms>` 或拆短轮次）。
2. **句间要断**：口头禅引导的转折/递进处（「好问题。→ 展开来说」「简单来说。→ 展开来说」）**必须用 `<break 300-400ms>` 断开**，别让两句连成一片。
3. **术语要"钉"**：重要专有名词、尤其代码里的函数名/类名（`execute_model`、`block_table` 等）首现点明「代码中的 xxx」，**必要时逐字母拼写**（execute_model → e-x-e-c-u-t-e 下划线 m-o-d-e-l），让读者"看得见"。
