...
### TTS 缺陷清单(2026-08-13 探针实测,IndexTTS-2.5)

- **字母大小写同音**:S/s、I/i、A/a 等全部读同音。脚本层加"大写/小写"前缀;合成器 `mark_letter_marks()` 处理孤立 S/s(B 站、说话人 A/B 不误伤)。
- **英文专名拼音式误读**:Python→"拍扇鞋"、UP 主→"二婆主"、KV cache→"KVCASH"。对策:发音表加条目(派森/U P 主/KV 缓存)。英文单词(bug/AI/B 站)读得准,数字(1960/1.5/29万/7:30)读得准。
- **法语不可靠**(读成英语/拼音式)→ 发音表替换为英语。
- **标点韵律被抹平**(「」→'、！→!):引号/感叹号无韵律作用,重读靠 {{em}}。
- **情绪参数改变发音气质**(惊讶→浮夸;轻高兴/文本情绪都不如现状):只做重读(放慢+响度),不做全局情绪。
- **标点停顿随机**(同一句号 0/60/270ms 三种):靠 <break> 拆段插静音。

### TTS 表达力:语速/字母记号(2026-08-13 定案)

- **全角色原速(SPEED=1.0)**:0.88/0.92 的降速会产生"人机感"——用户实测原速最好。语速/停顿/情感都别从全局参数调,模型自然输出最稳。
- **情绪向量注入全部失败**:emo_vector 轻高兴(0.15/0.25)、emo_text 文本情绪("自然地聊天…")都不如无情绪现状——情绪参数改变整句发音气质,只做重读({{em}} 放慢+响度,不改音色)。
- **2 vs 2.5 同音色对比无听感差异**:人机感来源不是模型,是管线参数(语速)。
- **图记号大小写区分**:IndexTTS 把大写 S 和小写 s 都读 /es/。合成前 `mark_letter_marks()` 把孤立 S→「大写 S」、s→「小写 s」(保护已有"大写/小写"前缀、只处理 S/s——B 站、说话人 A/B 不误伤)。发音表替换产物里的 S 也会展开("S 到 S 一撇"→"大写 S 到大写 S 一撇")。
- **概念讲解审查方法论(2026-08-13,ep02-09)**:六维审查(术语首现定义/直观性/例子承重/逻辑链/信息密度/口语化),输出 BLOCKING/WARN/亮点/术语评分表。全季 19 处 BLOCKING,共性:术语先点名后定义或从未定义、纯视觉记号口播无效、引文不转译、出处堆叠成"讲课腔"。亮点机制(老张 checkpoint 复述/阿哲复述循环/朋友圈七步走查)保持不动。

### TTS 表达力:重读/强调(2026-08-13 定案)

关键术语(「它思」等)需要重读时,**不要**指望标点或情绪向量:

- 文本前端把「」→'、！→! 抹平标点韵律;emo_vector/emo_text 是句级且**会改变整句发音气质**(惊讶 0.45 实测浮夸,"思"被读得不自然)
- calm 分量不控语速。语速只能后处理 atempo(WSOLA 保调,不伤音高)
- **最终配方(用户拍板)**:IndexTTS-2.5 G2P 注音(`<它思|TA1 SI1>` 读对)+ 放慢 3%(atempo 0.97)+ 响度 0dB(对齐同 turn 前段)+ 零情绪向量
- 脚本标记:`{{em}}它思{{/em}}`(writer 标重读词)→ 发音表 g2p 字段给注音 → 合成器处理强调段
- 实验方法:同核心句 × 多手段量化对比(时长/RMS/F0 峰值落差=重读音高成分)+ whisper 转写验读 + 用户试听定档
